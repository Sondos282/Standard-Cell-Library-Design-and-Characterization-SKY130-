...
def plot_inverter_delay():
    ...
    # 1. Filter for the inverter family
    inv_cells = ['invx1', 'invx2', 'invx4', 'invx8']
    
    # 2. Filter for the fixed input transition (tin = 0.1225ns)
    # Using np.isclose to avoid floating-point mismatch issues
    ...
    for cell in inv_cells:
        # Get data for this specific cell and sort by load capacitance
        cell_data = plot_data[plot_data['cell'] == cell].sort_values('cl_pf')
        
        if cell_data.empty:
            continue
            
        # 3. Calculate Average Delay: (cell_rise + cell_fall) / 2
        # Multiplying by 1000 to convert nanoseconds (ns) to picoseconds (ps) for a cleaner Y-axis
    ...
    # Formatting the plot to make it professional for your report
